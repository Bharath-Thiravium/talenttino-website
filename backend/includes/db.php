...
if (!defined('DB_CONNECT_TIMEOUT')) {
    define('DB_CONNECT_TIMEOUT', (int)$env('DB_CONNECT_TIMEOUT', '1'));
}
...
$conn = mysqli_init();
if ($conn instanceof mysqli && DB_CONNECT_TIMEOUT > 0) {
    $conn->options(MYSQLI_OPT_CONNECT_TIMEOUT, DB_CONNECT_TIMEOUT);
}
@$conn->real_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME, DB_PORT, $socket);
...
if ($conn->connect_error && $socket === null && in_array(DB_HOST, ['localhost', '::1'], true)) {
    ...
    @$conn->real_connect('127.0.0.1', DB_USER, DB_PASS, DB_NAME, DB_PORT);
}
